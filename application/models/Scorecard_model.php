...
class ScoreCard_model extends CI_Model {
  ...
  {
    $PerChartStats = 'SELECT chart_name, count(date), count(distinct date) FROM (SELECT date, chart_id FROM CHARTED NATURAL JOIN SONGS WHERE artist_id = ? UNION All SELECT date, chart_id FROM CHARTED NATURAL JOIN ALBUMS WHERE artist_id = ?)a NATURAL JOIN CHARTS GROUP BY chart_name';
  ...
  {
    $AllTimeDateStats = 'SELECT count(date), count(distinct date) FROM (SELECT date FROM CHARTED NATURAL JOIN SONGS WHERE artist_id = ? UNION ALL SELECT date FROM CHARTED NATURAL JOIN ALBUMS WHERE artist_id = ?)a';
  ...
  {
    $FirstAndLastApperancePerChart = 'SELECT chart_name, min(date) as FirstApperance, max(date) as MostRecentApperance FROM (SELECT date, chart_id FROM CHARTED NATURAL JOIN SONGS WHERE artist_id = ? UNION ALL SELECT date, chart_id FROM CHARTED NATURAL JOIN ALBUMS WHERE artist_id = ?)a NATURAL JOIN CHARTS GROUP by chart_id order by min(date) ASC';
